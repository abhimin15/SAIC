...
class FooNode(template.Node):
    ...
    def render(self, context):
        ...
        '''
        temp_category=obj.requestercategory
        if temp_category == 'alumni':
            temp_user=AluminiDetails.objects.get(email=obj.who_requested)
        elif temp_category == 'student':
            temp_user=StudentDetails.objects.get(email=obj.who_requested)
        elif temp_category == 'faculty':
            temp_user=FacultyDetails.objects.get(email=obj.who_requested)
        context['temp_user']=temp_user
        if obj.mobile_requested == '1':
            context['mobile_requested']= True
        elif obj.email_requested == '1':
            context['email_requested']= True
        return ''
