...
class FooNode(template.Node):
    ...
    def render(self, context):
        ...
        '''
        temp_category=obj.requestedcategory
        if temp_category == 'alumni':
            temp_user=AluminiDetails.objects.get(email=obj.whom_requested)
        elif temp_category == 'student':
            temp_user=StudentDetails.objects.get(email=obj.whom_requested)
        elif temp_category == 'faculty':
            temp_user=FacultyDetails.objects.get(email=obj.whom_requested)
        context['gen_user']=temp_user
        return ''

@register.tag
def do_requests(parser, token):
