...
def getMentorsDetails():
    ...
    f=open('/var/www/saic/SAIC/static/scripts/StudentDetailsFromFile'+time.strftime("%d-%m-%Y")+'.csv', 'w')
    writer =csv.writer(f, delimiter=',',quotechar='|',quoting=csv.QUOTE_MINIMAL)
    writer.writerow(["Email","First Name","Last Name","Gender","Department","Degrees","Year Of Passing","Date of Birth","Organisation","Mobile","Permanent Address","Permanent City","Permanent Zip","Permanent State","Permanent Country","Present State","Present Country","Present Address","Present City","Present Zip","Register"])
    mentees=open("/var/www/saic/SAIC/static/scripts/studentEmailFile.csv", 'r')
    count=0
    for y in mentees:
        t=y.split("\n")[0]
        for x in data:
